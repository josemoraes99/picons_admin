...
menuNewPiconShow.addEventListener('click', e => {
  ...
  document
    .querySelector('.newPiconsMenu')
    .parentElement.classList.add('active'); // show
...
menuListaCompleta.addEventListener('click', e => {
  ...
  document
    .querySelector('.listaCompletaMenu')
    .parentElement.classList.add('active'); // show
...
menuLogout.addEventListener('click', e => {
  ...
  saveToIndexedDB('objectstoreName', dbData);
...
const processLogin = async (usr, pwd) => {
  ...
  const response = await fetch(
    'https://1wdtecach7.execute-api.sa-east-1.amazonaws.com/prod/auth',
    {
      method: 'post',
...
const loadChannelsList = async () => {
  ...
  const filterList = channelTmp.filtroCanais;
...
const getUrlData = async payload => {
  ...
  if (authKey) {
    try {
      const response = await fetch(
        'https://1wdtecach7.execute-api.sa-east-1.amazonaws.com/prod',
        {
          method: 'post',
...
const processChannelList = chanArr => {
  ...
  listFile.unshift({
    // adicionando primeiro item vazio para o select box funcionar
    channell: ''
  });
    ...
    const curLi = createElement({
      ...
      childs: [
        ...
        createElement({
          ...
          className: 'col-sm-1',
          html:
            '<i class="far fa-trash-alt delete btnRemove" id="button.' +
            itemChannel.channell +
            '"></i>'
        })
      ]
    });
    ...
    const curLi = createElement({
      ...
      childs: [
        ...
        createElement({
          ...
          className: 'col-sm-1',
          html:
            itemChannel.stat != 'file'
              ? '<i class="fas fa-trash-restore delete btnRemoveRedir" id="button.' +
                itemChannel.channell +
                '"></i>'
              : ''
        })
...
const processFilterList = filterList => {
...
const afterDomChange = () => {
  const selectChannelWithFile = document.querySelectorAll(
    '.selectChannelWithFile'
  );
  ...
  // nodelist to array
  const selectCategorie = Array.from(
    document.querySelectorAll('.selectCategorie')
  );
